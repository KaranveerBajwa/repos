...
function sortArray(movies)
{
    ...
    for(let j = i + 1; j < movies.length;j++)
     {
         if(movies[j].rank > movies[maxIndex].rank )
         {
             maxIndex = j;
         }
     }
     swap(movies,i,maxIndex);
    }
    return movies;
}
...
function sortMoviesByAttr(movies, sortAttr){
    ...
    for(let j = i + 1; j < movies.length;j++)
     {
         if(movies[j][sortAttr] > movies[maxIndex][sortAttr])             
         {
             maxIndex = j;
         }
     }
